...
export default {
    ...
    methods:{
        resendCode() { 
            ...
            try {
                AuthService.resendCode(this.form).then((res)=>{
                    this.$toast(res.message);
                    setTimeout(() => {
                        useState('isBusy').value = false;
                        navigateTo('/auth/verify-email') // reset-password 
                    }, 2000);
                }).catch( (err) =>{
